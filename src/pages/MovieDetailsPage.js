...
const MovieDetailsPage = () => {
  ...
  return (
    <div className="py-10">
      ...
      <h1 className="mt-48 mb-10 text-4xl font-bold text-center text-white">
        {title}
      </h1>
      {genres.length > 0 && (
        <div className="flex items-center justify-center mb-10 gap-x-5">
          {genres.map((item) => (
            <span
              className="px-2 py-4 border rounded-lg border-primary text-primary"
              key={item.id}
            >
              {item.name}
            </span>
          ...
          <div className="flex flex-col gap-14">
            {results.length > 0 &&
              results.slice(0, 3).map((item) => (
                <div key={item.id}>
                  <h3 className="inline-block p-3 mb-5 text-xl font-medium bg-teal-800">
                    {item.name}
                  </h3>
